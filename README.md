...
| [linked-data](linked-data/SKILL.md) | Applies Tim Berners-Lee's linked data principles (four rules, dereferencing, browsable graphs, FOAF/seeAlso, 5-star linked open data). Use when designing or reviewing RDF publishing, HTTP URIs for things, 303 redirects, SPARQL endpoints, semantic web data linking, or explaining LOD and open data star ratings. |
| [microservices-skill](microservices-skill/SKILL.md) | Architect and implement production-grade microservices systems in TypeScript (NestJS) and Python (FastAPI), including resilience, observability, testing, deployment, and migration guidance. |
...
| [ts-otp-aws](ts-otp-aws/SKILL.md) | Implement OTP/passwordless authentication for TypeScript on AWS using Cognito CUSTOM_AUTH (default) or a custom DynamoDB flow, with SES/SNS delivery and security guardrails. |
...
| [ux-design-principles](ux-design-principles/SKILL.md) | Apply UX psychology and usability principles when building user interfaces, including Laws of UX, usability heuristics, and dark-pattern avoidance. |
